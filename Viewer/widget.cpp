...
void Widget::initTraceMenu()
{
    ...
    action = new QAction();
    traceMenu->addAction(action);
    traceMenu->addSeparator();

    action = new QAction("Приоритет");
    connect(action, &QAction::triggered,
            [this] () {
        emit activatedActionWithTrack(clickedTraceNumber, ActionsWithTrack::Priority);
        qDebug() << "Widget::initTraceMenu - id " << clickedTraceNumber;
    });
    traceMenu->addAction(action);

    action = new QAction("Продление");
    connect(action, &QAction::triggered,
            [this] () {
        emit activatedActionWithTrack(clickedTraceNumber, ActionsWithTrack::Extend);
    });
    traceMenu->addAction(action);

    action = new QAction("Сброс");
    connect(action, &QAction::triggered,
            [this] () {
        emit activatedActionWithTrack(clickedTraceNumber, ActionsWithTrack::Delete);
    });
    traceMenu->addAction(action);

    action = new QAction("Коррекция");
    connect(action, &QAction::triggered,
            [this] () {
        emit activatedActionWithTrack(clickedTraceNumber, ActionsWithTrack::Correction);
    });
    traceMenu->addAction(action);

    traceMenu->addSeparator();

    action = new QAction("Трек");
    connect(action, &QAction::triggered,
            [this] () {
        emit activatedActionWithTrack(clickedTraceNumber, ActionsWithTrack::Trace);
    });
    traceMenu->addAction(action);

    connect(traceMenu, &QMenu::aboutToHide, [this] () {
        preHiddingContextMenu = true;
        qDebug() << "QMenu::aboutToHide";
    });
}
